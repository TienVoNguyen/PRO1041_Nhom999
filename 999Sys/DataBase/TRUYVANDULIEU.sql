...
-- Bảng HOADON
/*
	SELECT HOADON.MAHOADON,MANV,MAKH,TENKHACHHANG,SDT,DIACHI,NGAYMUA,NGAYGIAOHANG,GIAMGIA,TIENSHIPHANG,THANHTIEN,TENTT,GIAOHANG.GHICHU
	FROM HOADON  JOIN GIAOHANG ON HOADON.MAHOADON = GIAOHANG.MAHOADON
						JOIN TT_HOADON ON TT_HOADON.MATT = GIAOHANG.MATT

	SELECT * FROM   HOADON join TT_HOADON on HOADON.MATT = TT_HOADON.MATT
	SELECT TENSP,CT_HOADON.SOLUONG FROM CT_HOADON JOIN SANPHAM on SANPHAM.MASP = CT_HOADON.MASP
	WHERE MAHD = 10001
...
SELECT * FROM GIAOHANG WHERE  (MAGIAOHANG = ?)
