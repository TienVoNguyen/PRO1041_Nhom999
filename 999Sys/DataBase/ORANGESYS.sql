...
END
GO
SELECT * FROM FNDTMONTH('2022-03-21')
select sum(thanhtien) from HOADON where Year(ngaymua) = 2022 and month(ngaymua) = 3
...
END
GO
SELECT * FROM FNDTYEAR('2022-03-21')
...
END
GO
SELECT dbo.FNSOHDDAYSINMONTH('2022-03-21') SOHD
...
END
GO
SELECT dbo.FNSOHDDAYSINYEARS(2022) DOANHTHU
...
END
GO
SELECT dbo.FNSOHDDAYSINYEAR(3, 2022) DOANHTHU
...
END
GO
SELECT dbo.FNDTDAYSINMONTH('2022-03-14') THANHTIEN
...
AS BEGIN
	SELECT
		...
		LEFT(CONVERT(VARCHAR(50),SUM(GIAMGIA),1),CHARINDEX('.',CONVERT(VARCHAR(50),SUM(GIAMGIA),1)) -1) + ' VNĐ' GIAMGIA,
		LEFT(dbo.FNDTDAYSINMONTH(NGAYMUA),CHARINDEX('.',dbo.FNDTDAYSINMONTH(NGAYMUA))-1) + ' VNĐ' AS DOANHTHU
	FROM HOADON HD
...
select ngaymua, thanhtien from HOADON where month(ngaymua) = 3 and year(ngaymua) = 2022 and MATT = 2
...
select * from HOADON where Year(ngaymua) = 2022
...
AS BEGIN
	SELECT
		...
		LEFT(CONVERT(VARCHAR(50),SUM(CT.SOLUONG * SP.GIABAN),1),CHARINDEX('.',CONVERT(VARCHAR(50),SUM(CT.SOLUONG * SP.GIABAN),1)) -1) + ' VNĐ' GIABAN,
		LEFT(CONVERT(VARCHAR(50),SUM(GIAMGIA),1),CHARINDEX('.',CONVERT(VARCHAR(50),SUM(GIAMGIA),1)) -1) + ' VNĐ' GIAMGIA,
		LEFT(dbo.FNSOHDDAYSINYEAR(MONTH(NGAYMUA), 2022),CHARINDEX('.',dbo.FNSOHDDAYSINYEAR(MONTH(NGAYMUA), 2022))-1) + ' VNĐ' AS DOANHTHU
	FROM HOADON HD
		JOIN CT_HOADON CT ON CT.MAHD = HD.MAHOADON
		JOIN SANPHAM SP ON SP.MASP = CT.MASP
...
AS BEGIN
	SELECT		
		...
		LEFT(CONVERT(VARCHAR(50),SUM(CT.SOLUONG * SP.GIABAN),1),CHARINDEX('.',CONVERT(VARCHAR(50),SUM(CT.SOLUONG * SP.GIABAN),1)) -1) + ' VNĐ' GIABAN,
		LEFT(CONVERT(VARCHAR(50),SUM(GIAMGIA),1),CHARINDEX('.',CONVERT(VARCHAR(50),SUM(GIAMGIA),1)) -1) + ' VNĐ' GIAMGIA,
		LEFT(dbo.FNSOHDDAYSINYEARS(YEAR(NGAYMUA)),CHARINDEX('.',dbo.FNSOHDDAYSINYEARS(YEAR(NGAYMUA)))-1) + ' VNĐ' AS DOANHTHU
	FROM HOADON HD
		JOIN CT_HOADON CT ON CT.MAHD = HD.MAHOADON
		JOIN SANPHAM SP ON SP.MASP = CT.MASP
		WHERE MATT = 2
...
--Tạo view xem Sản Phẩm còn lại
	IF OBJECT_id('SP_SP') is not null	--drop proc sp_timkiemgo--IF OBJECT_ID('SPTIMKIEM') IS NOT NULL
	DROP PROC SP_SP
	GO
	CREATE PROC SP_SP
	AS
