...
BEGIN	
	...
	IF (LEN(@GIATRI) > 2) 
		BEGIN
			SET @MONEY = (SELECT IIF(SP.APDUNGKM = 0, SP.GIABAN, IIF(SP.GIABAN - KM.GIATRI IS NULL, SP.GIABAN, SP.GIABAN - KM.GIATRI) )  FROM SANPHAM SP LEFT JOIN KHUYENMAI KM ON KM.MASP = SP.MASP WHERE SP.MASP = @MASP AND KM.TRANGTHAI =1)
		END
	ELSE 
		BEGIN
			SET @MONEY = (SELECT IIF(SP.APDUNGKM = 0, SP.GIABAN, IIF(SP.GIABAN * (KM.GIATRI /100)IS NULL, SP.GIABAN, SP.GIABAN - (SP.GIABAN * (KM.GIATRI/100))) )  FROM SANPHAM SP LEFT JOIN KHUYENMAI KM ON KM.MASP = SP.MASP WHERE SP.MASP = @MASP AND KM.TRANGTHAI = 1)
		END
	RETURN  @MONEY
END
GO
select IIF(dbo.FNSPKM(1004)IS NULL, GIABAN, dbo.FNSPKM(1004))  tien FROM SANPHAM
SELECT MASP, TENSP, MADM, MAVACH , IIF(dbo.FNSPKM(MASP) IS NULL, GIABAN, dbo.FNSPKM(MASP))  GIABAN, GIANHAP, SOLUONG, ANHSANPHAM, NGAYNHAP, APDUNGKM, TRANGTHAI, MACHATLIEU, MADVT, MAMAU, MASIZE  FROM   SANPHAM WHERE MADM like  ? and (MASP like ? or TENSP like ?)