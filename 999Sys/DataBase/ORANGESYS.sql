...
BEGIN	
	DECLARE @GIATRI INT
	SET @GIATRI = (SELECT CONVERT(INT, GIATRI,1) FROM KHUYENMAI WHERE MASP = @MASP AND TRANGTHAI = 1)

	DECLARE @TRANGTHAI BIT
	SET @TRANGTHAI = (SELECT TRANGTHAI FROM KHUYENMAI WHERE MASP = 1000 AND TRANGTHAI = 1)

	DECLARE @MONEY MONEY
	IF (LEN(@GIATRI) > 2) 
		BEGIN
			SET @MONEY = (SELECT IIF(SP.APDUNGKM = 0, SP.GIABAN, IIF(SP.GIABAN - KM.GIATRI IS NULL, SP.GIABAN, SP.GIABAN - KM.GIATRI) )  FROM SANPHAM SP LEFT JOIN KHUYENMAI KM ON KM.MASP = SP.MASP WHERE SP.MASP = 1000 AND KM.TRANGTHAI =1 AND KM.TRANGTHAI = 1 AND KM.NGAYBATDAU < CAST(GETDATE() AS DATE) AND CAST(GETDATE() AS DATE)  < KM.NGAYKETTHUC)
		END
	ELSE 
		BEGIN
			SET @MONEY = (SELECT IIF(SP.APDUNGKM = 0, SP.GIABAN, IIF(SP.GIABAN * (KM.GIATRI /100)IS NULL, SP.GIABAN, SP.GIABAN - (SP.GIABAN * (KM.GIATRI/100))) )  FROM SANPHAM SP LEFT JOIN KHUYENMAI KM ON KM.MASP = SP.MASP WHERE SP.MASP = 1000 AND KM.TRANGTHAI = 1 AND DAY(KM.NGAYKETTHUC) < DAY(CAST(GETDATE() AS DATE)))
		END
	RETURN  @MONEY
END
GO
select IIF(dbo.FNSPKM(1004)IS NULL, GIABAN, dbo.FNSPKM(1004))  tien FROM SANPHAM
select dbo.FNSPKM(1000)
SELECT DAY(CAST(GETDATE() AS DATE))
SELECT DATEDIFF(day, '2022-03-30', CAST(GETDATE() AS DATE))
SELECT MAKM, NGAYBATDAU, NGAYKETTHUC, MASP FROM KHUYENMAI WHERE NGAYBATDAU < CAST(GETDATE() AS DATE) AND CAST(GETDATE() AS DATE) <= NGAYKETTHUC