...
BEGIN	
	DECLARE @GIATRI INT
	SET @GIATRI = (SELECT CONVERT(INT, GIATRI,1) FROM SANPHAM SP LEFT JOIN KHUYENMAI KM ON KM.MASP = SP.MASP WHERE SP.MASP = @MASP AND KM.TRANGTHAI = 1 AND KM.NGAYBATDAU < CAST(GETDATE() AS DATE) AND CAST(GETDATE() AS DATE)  < KM.NGAYKETTHUC)
...
END
GO
select MASP , IIF(dbo.FNSPKM(MASP)IS NULL, GIABAN, dbo.FNSPKM(MASP))  tien FROM SANPHAM
