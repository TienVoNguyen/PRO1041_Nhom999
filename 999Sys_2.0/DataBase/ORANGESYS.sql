...
BEGIN
	DECLARE @SOHD INT
	SET @SOHD = (SELECT COUNT(MAHOADON) FROM HOADON WHERE CAST(NGAYMUA AS DATE) = @DAY AND MATT = 2)
	RETURN @SOHD
END
GO
...
BEGIN
	DECLARE @DOANHTHU MONEY
	SET @DOANHTHU = (SELECT SUM(THANHTIEN) FROM HOADON WHERE CAST(NGAYMUA AS DATE) = @DAY AND MATT = 2)
	RETURN CONVERT(VARCHAR(50),@DOANHTHU,1)
END
GO
...
AS BEGIN
	SELECT
		...
		LEFT(CONVERT(VARCHAR(50),SUM(CT.GIABAN),1),CHARINDEX('.',CONVERT(VARCHAR(50),SUM(CT.GIABAN),1)) -1) GIABAN,
...
GO
	...
	CREATE PROC SP_TimKiemHD
		@MaNV nvarchar(50),@MaKH nvarchar(50)
	AS BEGIN 
		SELECT DISTINCT HOADON.MAHOADON,HOADON.MANV,HOTEN,
		IIF(MAKH is null,N'KL',MAKH)as MaKH,NGAYMUA,GIAMGIA,THANHTIEN,TENTT,GHICHU
		FROM HOADON join TAIKHOAN on HOADON.MANV = TAIKHOAN.MANV
			join TT_HOADON on TT_HOADON.MATT = HOADON.MATT
		WHERE MAHOADON NOT IN (SELECT MAHOADON FROM GIAOHANG) and
				(HOADON.MANV like @MaNV and(
				HOADON.MAKH like @MaKH or HOADON.MAKH is null) )
	END
	GO
	--{CALL SP_TimKiemHD()}
	...
	WHERE HOADON.MANV like @MaNV and(
		HOADON.MAKH like @MaKH or HOADON.MAKH is null)
	END
	--{CALL SP_TimKiemHDGiaoHang()}

GO
IF OBJECT_ID('SP_UPDATEGIAOCA') IS NOT NULL
		DROP PROC SP_UPDATEGIAOCA
	GO
	CREATE PROC SP_UPDATEGIAOCA
	AS BEGIN 
		DECLARE @SOCACHOGIAO INT
		SET @SOCACHOGIAO = (SELECT COUNT(MATT) FROM GIAOCA WHERE MATT = 2)
		IF @SOCACHOGIAO >= 2 
		BEGIN
			UPDATE GIAOCA SET MATT = 0 WHERE MATT = 2
		END
		ELSE 
		BEGIN
			UPDATE GIAOCA SET MATT = 0 WHERE MATT = 2 AND DATEDIFF(HOUR, GIONHANCA, GETDATE()) >= 24
		END
	END

GO
IF OBJECT_ID('SP_TIMKIEMGIAOCA') IS NOT NULL
		DROP PROC SP_TIMKIEMGIAOCA 
	GO
	CREATE PROC SP_TIMKIEMGIAOCA (@MANVGC VARCHAR, @MANVNC VARCHAR, @MATT INT)
	AS BEGIN 
		IF @MATT IS NULL
			BEGIN
				SELECT 'CA' + CAST(MAGIAOCA AS VARCHAR) as MAGIAOCA, TK.HOTEN NHANVIENGC, IIF(TK1.HOTEN IS NULL, N'Chưa có', TK1.HOTEN) NHANVIENNC, CONVERT(VARCHAR,GIONHANCA) as GIONHANCA, IIF(GIOGIAOCA IS NULL, N'Chưa giao', CONVERT(VARCHAR,GIOGIAOCA)) as GIOGIAOCA, TIENCOSO, TIENPHATSINH, DOANHTHUCA, TONGTIEN, IIF(GHICHUGIAO IS NULL, N'Chưa có', GHICHUGIAO) GHICHUGIAO, IIF(GHICHUNHAN IS NULL, N'Chưa có', GHICHUNHAN) GHICHUNHAN, TTGC.TENTT 	FROM GIAOCA GC JOIN TAIKHOAN TK ON TK.MANV = GC.MANVGIAOCA JOIN TTGIAOCA TTGC ON TTGC.MATT = GC.MATT  LEFT JOIN TAIKHOAN TK1 ON TK1.MANV = GC.MANVNHANCA WHERE (MANVGIAOCA LIKE '%' + @MANVGC + '%' OR MANVNHANCA LIKE '%' + @MANVNC + '%')
			END
		ELSE 
			BEGIN
				SELECT 'CA' + CAST(MAGIAOCA AS VARCHAR) as MAGIAOCA, TK.HOTEN NHANVIENGC, IIF(TK1.HOTEN IS NULL, N'Chưa có', TK1.HOTEN) NHANVIENNC, CONVERT(VARCHAR,GIONHANCA) as GIONHANCA, IIF(GIOGIAOCA IS NULL, N'Chưa giao', CONVERT(VARCHAR,GIOGIAOCA)) as GIOGIAOCA, TIENCOSO, TIENPHATSINH, DOANHTHUCA, TONGTIEN, IIF(GHICHUGIAO IS NULL, N'Chưa có', GHICHUGIAO) GHICHUGIAO, IIF(GHICHUNHAN IS NULL, N'Chưa có', GHICHUNHAN) GHICHUNHAN, TTGC.TENTT 	FROM GIAOCA GC JOIN TAIKHOAN TK ON TK.MANV = GC.MANVGIAOCA JOIN TTGIAOCA TTGC ON TTGC.MATT = GC.MATT  LEFT JOIN TAIKHOAN TK1 ON TK1.MANV = GC.MANVNHANCA WHERE (MANVGIAOCA LIKE '%' + @MANVGC + '%' OR MANVNHANCA LIKE '%' + @MANVNC + '%') AND GC.MATT = @MATT
			END
	END
	GO 
	-- update loai khach hang
	IF OBJECT_ID('updateKieuKhachHang01') is not null
	DROP TRIGGER updateKieuKhachHang01
GO
CREATE TRIGGER updateKieuKhachHang01 ON KHACHHANG
FOR UPDATE
AS
BEGIN
	DECLARE @DiemEXP int;
	SELECT @DiemEXP = KHACHHANG.DIEMEXP FROM KHACHHANG, inserted where inserted.MAKH = KHACHHANG.MAKH
	IF @DiemEXP > 5000 
	BEGIN 
	UPDATE KHACHHANG SET MALOAIKH = 2 From inserted,KHACHHANG WHERE inserted.MaKH = KHACHHANG.MAKH
	END
	ELSE
	BEGIN
		UPDATE KHACHHANG SET MALOAIKH = 1 From inserted,KHACHHANG WHERE inserted.MaKH = KHACHHANG.MAKH
	END
END
